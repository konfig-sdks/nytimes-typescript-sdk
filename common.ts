...
import { readableStreamToString, NytError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NytError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NytError(e, e.response?.data)
                }
                if (innerError instanceof NytError) {
                    // Got "NytError" from the above try block
                    throw innerError;
                }
